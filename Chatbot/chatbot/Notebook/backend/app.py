# chatbot/Notebook/backend/app.py
...
def login(): # Changed /login to /index to match the form action
    ...
    if request.method == 'POST':
        ...
        if user and brcrypt.check_password_hash(user['password'], password):
            ...
            flash(error) # Use flash for error messages
            # Return to login page with error, do not redirect to index again
...
def dashboard():
    ...
    flash("Please login to access the dashboard.") # Message for direct access attempt
    return redirect(url_for('login')) # Redirect to login if not in session
...
def analyze_document():
    ...
    try:
        ...
        logger.debug(f"Raw 'analysis_content' (first 300): '''{analysis_content[:300] if analysis_content else 'None'}...'''")
        logger.debug(f"Raw 'thinking_content' (first 300): '''{thinking_content[:300] if thinking_content else 'None'}...'''")
        logger.debug(f"--- End Analysis for Frontend ---")

        if analysis_content is None:
             error_msg = f"Analysis failed: Could not retrieve or process '{filename}'."
             status_code = 404 
            ...
            status_code = 500 
            if "not found" in error_message.lower(): status_code = 404
            elif "AI model failed" in error_message or "AI model is not available" in error_message: status_code = 503 
        ...
        logger.error(f"Unexpected error in /analyze for '{filename}' ({analysis_type}): {e}", exc_info=True)
        return jsonify({"error": f"Unexpected server error during analysis: {type(e).__name__}.", "thinking": None}), 500
...
def chat():
    if not app_db_ready:
        ...
        return jsonify({
            ...
            "answer": "Cannot process chat, the database is currently unavailable.",
            "thinking": None, "references": [], "session_id": None, "web_search_payload": None
        }), 503 
    ...
    if not app_ai_ready or not ai_core.llm or not ai_core.embeddings:
        ...
        return jsonify({
            ...
            "answer": "Cannot process chat, the AI components are not ready.",
            "thinking": None, "references": [], "session_id": None, "web_search_payload": None
        }), 503 

    if not app_vector_store_ready and config.RAG_CHUNK_K > 0: 
        logger.warning("Chat: vector store not loaded/ready. RAG context will be empty.")
    ...
    if session_id:
        try: uuid.UUID(session_id, version=4)
        except (ValueError, TypeError, AttributeError):
            logger.warning(f"Invalid session_id format: '{session_id}'. Generating new.")
            session_id = str(uuid.uuid4()); is_new_session = True
    else:
        session_id = str(uuid.uuid4()); is_new_session = True
    ...
    try:
        if not database.save_message(session_id, 'user', query, None, None):
             logger.error(f"Failed to save user message to DB for session {session_id}.")
    except Exception as db_err:
         logger.error(f"DB error saving user message for session {session_id}: {db_err}", exc_info=True)

    bot_answer, references, thinking_content = "Sorry, an issue occurred.", [], None 
    web_search_results_text_for_llm = None # This goes to LLM
    web_search_payload_for_frontend = None # This goes to frontend

    try:
        context_text, context_docs_map = "No specific document context retrieved.", {}
        if app_vector_store_ready and config.RAG_CHUNK_K > 0:
            logger.debug(f"Performing RAG search (session: {session_id})...")
            _, context_text, context_docs_map = ai_core.perform_rag_search(query) 
            logger.info(f"RAG search found {len(context_docs_map)} unique context sources for session {session_id}.")
        elif not app_vector_store_ready and config.RAG_CHUNK_K > 0:
             context_text = "Knowledge base access unavailable; providing general answer."
        else: 
             context_text = "Document search disabled; providing general answer."

        if config.WEB_SEARCH_ENABLED:
            logger.debug(f"Performing web search (session: {session_id})...")
            # The result from perform_web_search is suitable for both LLM and frontend display
            web_search_results_text_for_llm = ai_core.perform_web_search(query)
            web_search_payload_for_frontend = web_search_results_text_for_llm # Use the same formatted text
            if web_search_results_text_for_llm:
                logger.info(f"Web search completed for session {session_id}.")
            else:
                logger.info(f"Web search yielded no results or was skipped for session {session_id}.")
        
        logger.debug(f"Synthesizing chat response (session: {session_id})...")
        bot_answer, thinking_content = ai_core.synthesize_chat_response(query, context_text, web_search_results_text_for_llm)
        ...
        if context_docs_map and not (bot_answer.startswith("Error:") or "[AI Response Processing Error:" in bot_answer):
            logger.debug(f"Extracting RAG references (session: {session_id})...")
            references = utils.extract_references(bot_answer, context_docs_map)
            if references: logger.info(f"Extracted {len(references)} unique RAG references for session {session_id}.")
        else:
             logger.debug(f"Skipping RAG reference extraction for session {session_id}: No context map or bot error.")

        try:
            if not database.save_message(session_id, 'bot', bot_answer, references, thinking_content): # web_search_payload is not saved to DB
                 logger.error(f"Failed to save bot response to DB for session {session_id}.")
        except Exception as db_err:
             logger.error(f"DB error saving bot response for session {session_id}: {db_err}", exc_info=True)
        ...
        return jsonify({
            ...
            "web_search_payload": web_search_payload_for_frontend # Add this field
        }), 200 

    except Exception as e:
        logger.error(f"Unexpected error in chat pipeline (session {session_id}): {e}", exc_info=True)
        error_message = f"Sorry, an unexpected server error occurred ({type(e).__name__})."
        try:
            database.save_message(session_id, 'bot', error_message, None, f"Error in /chat: {type(e).__name__}: {str(e)}")
        except Exception as db_log_err:
            logger.error(f"Failed to save error message to DB (session {session_id}): {db_log_err}")
        return jsonify({
            "error": "Unexpected server error.", "answer": error_message,
            "session_id": session_id, "thinking": f"Error in /chat: {type(e).__name__}", 
            "references": [], "web_search_payload": None
        }), 500
...
def get_history():
    ...
    try:
        ...
        # Note: web_search_payload is not stored in history, so it won't be present here.
        # This is by design, as web search is per-query, not part of historical messages.
        if messages is None:
         ...
         logger.error(f"Unexpected error in /history for session {session_id}: {e}", exc_info=True)
         return jsonify({"error": f"Unexpected server error retrieving history: {type(e).__name__}."}), 500
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not (1024 <= port <= 65535):
             logger.warning(f"Port {port} out of range (1024-65535). Using 5000.")
             port = 5000
    except ValueError:
        port = 5000
        logger.warning(f"Invalid FLASK_RUN_PORT. Using default {port}.")

    host = '0.0.0.0'
    logger.info(f"--- Starting Waitress WSGI Server ---")
    logger.info(f"Serving Flask app '{app.name}' on http://{host}:{port}")
    logger.info(f"Ollama URL: {config.OLLAMA_BASE_URL}, LLM: {config.OLLAMA_MODEL}, Embed: {config.OLLAMA_EMBED_MODEL}")
    logger.info(f"Web Search: {'Enabled' if config.WEB_SEARCH_ENABLED else 'Disabled'}")
    
    db_status = 'Ready' if app_db_ready else 'Failed'
    ai_status = 'Ready' if app_ai_ready else 'Failed'
    idx_status = 'Loaded' if app_vector_store_ready else ('Not Found' if app_ai_ready else 'Not Loaded (AI Fail)')
    cache_status = f"{len(ai_core.document_texts_cache)} docs" if app_doc_cache_loaded else "Failed"
    logger.info(f"Components: DB={db_status} | AI={ai_status} | Index={idx_status} | DocCache={cache_status}")
