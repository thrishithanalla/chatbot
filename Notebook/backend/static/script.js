...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateControlStates() {
        ...
        if (voiceInputButton) { // Updated for MediaRecorder
            voiceInputButton.disabled = !canChat; 
            voiceInputButton.title = canChat ? "Start Voice Input" : "Chat disabled for voice input";
        }
    ...
    function setupEventListeners() {
        ...
        if (voiceInputButton) voiceInputButton.addEventListener('click', toggleRecording); // Changed from toggleListening
...
.replace(/'/g, "&#39;");
    ...
    function disableChatInput(disabled = true) {
        ...
        if (voiceInputButton) voiceInputButton.disabled = disabled; // Simplified, enabled if chat is enabled
    }
    ...
    async function toggleRecording() {
        if (!voiceInputButton || voiceInputButton.disabled) return;
        ...
        if (isRecording) {
            ...
            stopRecordingUI("Processing..."); // Indicate processing
        } else {
            // Start recording
            if (synth && synth.speaking) {
            ...
            try {
                ...
                mediaRecorder.onstop = async () => {
                    ...
                    try {
                        ...
                        setChatStatus(`Transcription error: ${transcriptionError.message.substring(0,50)}...`, 'danger');
                        stopRecordingUI(); // Reset to default state
                    } finally {
                        audioChunks = []; // Clear chunks for next recording
                        // Release microphone resources by stopping tracks on the stream
                        if (stream) {
                ...
                setChatStatus("Mic access error. Check permissions?", 'warning');
                stopRecordingUI();
                // Ensure tracks are stopped if stream was partially obtained
    ...
    function startRecordingUI() {
        isRecording = true;
        if (voiceInputButton) {
            voiceInputButton.classList.add('recording', 'btn-danger');
    ...
    function stopRecordingUI(statusText = null) {
        isRecording = false;
        if (voiceInputButton) {
            voiceInputButton.classList.remove('recording', 'btn-danger');
        ...
        } else if (chatStatus && (chatStatus.textContent === 'Recording...' || chatStatus.textContent === 'Processing...')) {
             setChatStatus(backendStatus.ai ? 'Ready' : 'AI Offline', backendStatus.ai ? 'muted' : 'warning'); 
        }
    }
