...
import tempfile # Added for Whisper API
...
def transcribe_audio():
    ...
    if not config.OPENAI_API_KEY or config.OPENAI_API_KEY == "your_openai_api_key_here":
        ...
        return jsonify({"error": "Audio transcription service not configured by administrator (missing API key)."}), 501 # Not Implemented / Service Unavailable

    if 'audio_data' not in request.files:
        logger.warning("Transcription request missing 'audio_data' part.")
        return jsonify({"error": "No audio data in the request"}), 400

    audio_file = request.files['audio_data']
    if not audio_file or not audio_file.filename:
        logger.warning("Transcription request received with no selected audio file name.")
        return jsonify({"error": "No audio file selected"}), 400

    # Save the blob to a temporary file because the OpenAI API client expects a file path or file-like object
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".webm") as tmp_audio_file: # Assuming webm, common for browser MediaRecorder
            audio_file.save(tmp_audio_file.name)
            tmp_audio_path = tmp_audio_file.name
        logger.debug(f"Temporary audio file saved at: {tmp_audio_path}")

        client = openai.OpenAI(api_key=config.OPENAI_API_KEY)
        with open(tmp_audio_path, "rb") as audio_for_api:
            transcript_response = client.audio.transcriptions.create(
                model="whisper-1", # Standard OpenAI Whisper API model
                file=audio_for_api
            )
        transcribed_text = transcript_response.text
        logger.info(f"Audio transcribed successfully by Whisper API. Length: {len(transcribed_text)}")
        return jsonify({"transcript": transcribed_text})

    except openai.APIError as e:
        logger.error(f"OpenAI API error during transcription: {e}", exc_info=True)
        return jsonify({"error": f"Speech-to-text API error: {e.message if hasattr(e, 'message') else str(e)}"}), 502 # Bad Gateway
    except Exception as e:
        logger.error(f"Error processing audio for transcription: {e}", exc_info=True)
        return jsonify({"error": f"Server error during audio processing: {type(e).__name__}"}), 500
    finally:
        if 'tmp_audio_path' in locals() and os.path.exists(tmp_audio_path):
            os.remove(tmp_audio_path)
            logger.debug(f"Temporary audio file {tmp_audio_path} deleted.")
